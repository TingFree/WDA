...
def read_tencent_data(file, test=False):
    data = read_txt(file)
    target, labels = [], []
    for ex in data:
        text_a, text_b, label = ex.split('\t')
        if test:
            target.append([text_a, text_b])
        else:
            target.append([text_a, text_b, int(label)])
...
def test(model, dataloader, device):
    ...
    with torch.no_grad():
        for (batch_seqs, batch_seq_masks, batch_seq_segments, batch_labels) in dataloader:
            ...
            preds.extend(probabilities.argmax(dim=1).cpu().numpy().tolist())
    ...
    return batch_time, total_time, accuracy, roc_auc_score(all_labels, all_prob), preds


def train(model, dataloader, optimizer, epoch_number, max_gradient_norm, device):
    """
